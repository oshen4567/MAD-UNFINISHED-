...
import lk.nibm.holidayapp.MainActivity.HolidayHolder as HolidayHolder1
...
class MainActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        //setting  values into spinner (spnYear)
        ...
        btn_Search.setOnClickListener {
            val  img = spinner_Country.selectedItem.toString()
            val ii = spinner_Year.selectedItem.toString()
            Toast.makeText(applicationContext,"Country "+img+" year "+ii,Toast.LENGTH_SHORT).show()


        }

    }
    //Getting all the countries and setting those values into spinner (spnCountry)
    private fun getCountryDetails(){
        ...
        val request = StringRequest(Request.Method.GET,url,
            Response.Listener { respose ->
                try{
                    countryArrayFullDetails = JSONArray(respose)
                    countrySpinnerArray = Array<String>(countryArrayFullDetails.length()){"s"}

                    for(i in 0 until countryArrayFullDetails.length()){
                        countrySpinnerArray[i] = countryArrayFullDetails.getJSONObject(i).getJSONObject("name").getString("common")
                    }
    ...
    fun getHolidaydata(selectedYear:String){

        val url = "https://calendarific.com/api/v2/holidays?&api_key=a7b72618c0caec4ccc77e19322ad03b566e3a556&country=in&year=$selectedYear&"
